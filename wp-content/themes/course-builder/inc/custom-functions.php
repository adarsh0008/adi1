...
function thim_page_title( $output_value = null ) {
	...
	if ( is_singular( 'post' ) ) {
		$output['show_text'] = get_theme_mod( 'blog_single_pagetitle' );
	}
			...
			if ( get_post_type() == "lp_course" || get_post_type() == "lp_collection" ) {
				...
				if ( get_post_type() == "lp_course" ) {
					if ( $excerpt_content ) {
						...
						$main_content       = get_extended( $wp_query->queried_object->post_content );
						$custom_description = $main_content['main'];
					}
					...
					if ( $single_layout == 1 ) {
						...
						if ( $price = $course->get_price_html() ) {
							...
							$output_price_html = ob_get_contents();
							ob_end_clean();

							$custom_description .= $output_price_html;
