...
function thim_entry_meta_comment_number() {
	if ( comments_open() ) { ?>
		<span class="comment-total">
			<?php comments_popup_link( 'No Comment', '1 Comment', '% Comments', 'comments-link', 'Comments are off for this post' ); ?>
		</span>
		<?php
	}
}
...
function thim_render_social_share_link( $social_name ) {
	switch ( $social_name ) {
		...
		case 'pinterest':
			...
			return '<li><a class="link pinterest" title="' . esc_html__( 'Pinterest', 'construction-wp' ) . '" href="http://pinterest.com/pin/create/button/?url=' . urlencode( get_permalink() ) . '&amp;media=' . ( ! empty( $src[0] ) ? $src[0] : '' ) . '&description=' . esc_attr( urlencode( get_the_title() ) ) . '" onclick="window.open(this.href, \'mywin\',\'left=50,top=50,width=600,height=350,toolbar=0\'); return false;"><i class="ion-social-pinterest" aria-hidden="true"></i></a></li>';
			break;

		case 'linkedin':
			return '<li><a class="link linkedin" title="' . esc_html__( 'LinkedIn', 'construction-wp' ) . '" href="http://www.linkedin.com/shareArticle?title=' . esc_attr( urlencode( get_the_title() ) ) . '&amp;url=' . esc_url( urlencode( get_permalink() ) ) . '" rel="nofollow" onclick="window.open(this.href,this.title,\'width=600,height=600,top=200px,left=200px\');  return false;" target="_blank"><i class="ion-social-linkedin"></i></a></li>';
			break;


		case 'google':
			return '<li><a class="link google" title="' . esc_html__( 'Google', 'construction-wp' ) . '" href="https://plus.google.com/share?url=' . esc_url( urlencode( get_permalink() ) ) . '" rel="nofollow" onclick="window.open(this.href,this.title,\'width=600,height=600,top=200px,left=200px\');  return false;" target="_blank"><i class="ion-social-googleplus"></i></a>';
			break;
...
if ( ! function_exists( 'thim_archive_title' ) ) :
	function thim_archive_title( $before = '', $after = '' ) {
		if ( is_post_type_archive( ) ) {
			...
			$title = sprintf( esc_html__( '%1$s: %2$s', 'course-builder' ), $tax->labels->singular_name, single_term_title( '', false ) );
		} elseif ( is_404() ) {
