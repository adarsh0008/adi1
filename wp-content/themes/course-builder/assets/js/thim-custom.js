...
(function ($) {
	...
	var thim_course_builder = window.thim_course_builder = {
		...
		ready: function () {
			...
			this.thim_fix_vc_full_width_row();
		...
		magic_line: function () {
			...
			if ($(window).width() > 768) {
				...
				$('body.rtl').each(function () {
					$magicLine
						.width($current_a.width())
						.css("left", $current.position().left + parseInt($current_a.css('padding-left')) + 23)
						.data("origLeft", $current.position().left + parseInt($current_a.css('padding-left')) + 23)
						.data("origWidth", $current_a.width());
				});
		...
		/**
		 * add_loading_enroll_button
		 */
		add_loading_enroll_button: function () {
            $("form.enroll-course .button-enroll-course").on('click', function () {
		...
		* */
        ...
        /**
         * Event Grid, List switch
         */
        thim_event_switcher: function () {

            var activeClass = 'switcher-active';
            var gridClass = 'event-grid';
            var listClass = 'event-list';
            $('.eventToList').on ('click',function () {
                if (!$.cookie('events_page') || $.cookie('events_page') == 'grid') {
                    eventToList();
                }
            });

            $('.eventToGrid').on ('click',function () {
                if (!$.cookie('events_page') || $.cookie('events_page') == 'list') {
                    eventToGrid();
                }
            });

            function eventToList() {
                $('.eventToList').addClass(activeClass);
                $('.eventToGrid').removeClass(activeClass);
                $('.thim-list-event').fadeOut(300, function () {
                    $(this).removeClass(gridClass).addClass(listClass).fadeIn(300);
                    $.cookie('events_page', 'list', {expires: 3, path: '/'});
                });
            }

            function eventToGrid() {
                $('.eventToGrid').addClass(activeClass);
                $('.eventToList').removeClass(activeClass);
                $('.thim-list-event').fadeOut(300, function () {
                    $(this).removeClass(listClass).addClass(gridClass).fadeIn(300);
                    $.cookie('events_page', 'grid', {expires: 3, path: '/'});
                });
            }

            if ($.cookie('events_page') == 'grid') {
                $('.thim-list-event').removeClass('event-list').addClass('event-grid');
                $('.eventToGrid').addClass(activeClass);
            } else if ($.cookie('events_page') == 'list') {
                $('.thim-list-event').removeClass('evevnt-grid').addClass('event-list');
                $('.eventToList').addClass(activeClass);
            }
            else {
                $('.eventToGrid').addClass(activeClass);
